...
vector *vector_create(copy_constructor_type copy_constructor,
 ...
 new_vec->size = 0;
 new_vec->capacity = get_new_capacity(INITIAL_CAPACITY);
 new_vec->array = malloc(new_vec->capacity*sizeof(void*));
...
void vector_resize(vector *this, size_t n) {
    assert(this);
    size_t current_cap = this->capacity;
...
void vector_insert(vector *this, size_t position, void *element) {
    ...
    if (position>=this->size)
		position=this->size;
	vector_resize(this,this->size+1);
	void** arr = malloc(sizeof(void*)* (this->capacity+1) );
	for(size_t i = 0; i <position; i ++ ){
  			arr[i]=this->array[i];

  			}
	 arr[(int)position]=element;
	for(size_t i =position; i < this->size-1; i ++ ){
  			arr[i+1] = this->array[i];
  		}
	for(size_t i=0; i<this->size;i++)
		this->array[i]=this->copy_constructor(arr[i]);
	free(arr);
  arr=NULL;
}
...
void** arr = malloc(sizeof(void*)* this->capacity );
for(size_t i = 0; i <position; i ++ ){
  arr[i]=this->array[i];
  }
